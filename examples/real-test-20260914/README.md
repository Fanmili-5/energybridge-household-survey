...
- [初步监督字段清洗 JSON](cleaned-supervision.json)：顶层只含 `input` 和 `output`。输入为家庭画像的英文结果值、事件条件、No-DR 计划和 EB Agent 计划；输出是真人选择、四项评分与原文原因。不含问卷题干、作答状态、EP 结果、来源元数据、SFT 提示词或训练对话。
