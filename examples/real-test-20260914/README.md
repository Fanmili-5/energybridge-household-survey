...
- [第一步清洗后的监督数据 JSON](cleaned-supervision.json)：家庭输入、结构化对比与真人目标，另存补充信息；没有 SFT 提示词或训练对话。
...
清洗数据是对这条真实记录的新转换结果，并非当时已保存的训练文件。对比数值采用当时展示给参与者的精度；不把隐藏的 EB 推理、预测接受率或未展示的备注加入输入。英文转换不改写原始中文记录。
