...
定时快照包含数据库、问题报告和案例文档，不复制 EP 轨迹；需要备份已结束案例（含失败）的运行附件时，使用 `realtime_pilot/backup_backend.py`。备份完成后核对清单，并在独立目录测试恢复。跨机器保存备份可避免主机或磁盘故障同时损坏原始数据与备份。

案例编号、用户报告、管理员核查及诊断导出见[问题反馈说明](CASE_REPORTS.md)。升级时数据库自动添加报告表，不改变已有家庭资料、案例和评分。
