...
| 数据与 SFT | 通过 | 原始回答、规范化回答、题表/情境/方案/展示/反馈及哈希分层保存；SFT user 不含真人目标，assistant 才包含标签；所有候选仍为 `training_release=false`。 |
...
- SFT messages：system 定义家庭方案判断任务；user 只包含实际家庭回答和参与者看到的完整比较；assistant 是该家庭的真人选择、评分与原因。
