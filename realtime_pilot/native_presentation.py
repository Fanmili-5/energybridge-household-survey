...
def metrics(baseline, proposal, scenario):
    ...
    def summarize(run):
        daily=sum(r['kwh'] for r in run['electricity'] if 0<r['end_h']<=24)
...
def display(original, baseline, proposal, scenario, prediction):
    def intervals(run, device):
        ...
        for row in run['controls']:
            start=max(0,row['start_h']);end=min(24,row['end_h'])
    ...
    return {'title':'日常对照与 EB 调整安排','context':scenario,'prediction':prediction,'rows':rows,
        ...
        'notice':'日常对照按您提供的设备和时间窗口，由原 EB 生成；实际时间以图为准。两条路径从模拟开始分别运行，比较事件日00:00—24:00的结果。原 EB 可提前规划，不以问卷中的某个时刻作为首次获知事件的时间。跨夜任务只展示截至24:00的状态。',
