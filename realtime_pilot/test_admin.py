    ...
    def test_role_quotas_data_origin_and_public_isolation(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            def intake(admin=False,cookie='a'*64):
                return call('/api/households',{'questionnaire_context_hash':call('/api/session',admin=admin,cookie=cookie)[1]['questionnaire_context']['context_hash'],'request_id':'admin_intake_test_00001','answers':answers(),'questionnaire_version':QUESTIONNAIRE_VERSION,'questionnaire_hash':digest(QUESTIONS),'research_consent':True,'scenario_understood':True,'research_notice_version':'eb.research_notice.v2','ui_version':PARTICIPANT_UI_VERSION},admin,cookie)[1]['id']
