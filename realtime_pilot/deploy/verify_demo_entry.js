...
(async()=>{
 ...
 try{
  ...
  for(const width of [390,1365]){
   ...
   await page.waitForFunction(()=>document.getElementById('collection-badge').textContent==='演示试用');
   ...
   assert((await page.locator('.preview-note').innerText()).includes('两份安排相同'));
   assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
   await page.locator('header a').last().click();await page.locator('#wizard-nav').waitFor();
   for(const [code,message] of [[401,'登录已失效'],[429,'当前请求较多'],[502,'计算服务暂时不可用']]){
