...
(async()=>{
 ...
 try{
  let x=await setup('paused'),p=x.page;await p.locator('[data-question-id]').first().waitFor();assert(await p.locator('#generate').isEnabled());assert.strictEqual(await p.locator('#generate').innerText(),'保存家庭资料');assert(await p.locator('#planning-status').isVisible());await p.locator('#new-case').click();assert(await p.locator('#generate').isEnabled());reports.push({case:'paused',passed:true,...x.state()});await p.screenshot({path:out+'/paused-mobile.png'});assert.deepStrictEqual(x.errors,[]);await x.ctx.close();
