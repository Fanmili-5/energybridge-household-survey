...
(async()=>{
 ...
 async function resume(page){await page.locator('#resume-result').waitFor({state:'visible'});await page.locator('#resume-result').click();}
 try{
  ...
  x=await setup('feedback');p=x.page;await resume(p);await p.locator('#decision-form').waitFor({state:'visible'});
  ...
  await p.reload();await resume(p);await p.locator('#decision-form').waitFor({state:'visible'});assert.strictEqual(await p.locator('[name=feedback_score]').inputValue(),'3.75');assert.strictEqual(await p.locator('#decision-reason').inputValue(),'工程测试草稿');assert(await p.locator('[name=decision][value=reject]').isChecked());
  // Same browser, different result ID must not borrow this draft.
  x.alternate(true);await p.reload();await resume(p);await p.locator('#decision-form').waitFor({state:'visible'});assert.strictEqual(await p.locator('[name=feedback_score]').inputValue(),'');x.alternate(false);await p.reload();await resume(p);await p.locator('#decision-form').waitFor({state:'visible'});
  await p.waitForTimeout(100);const pan=await p.evaluate(()=>{let s=document.querySelector('.schedule-scroll'),e=s.querySelector('.event-shade').getBoundingClientRect(),r=s.getBoundingClientRect(),label=s.querySelector('.schedule-device').getBoundingClientRect();const heading=s.querySelector('.schedule-group h3 strong').getBoundingClientRect();return {headingLeft:heading.left,headingRight:heading.right,scroll:s.scrollLeft,contentWidth:s.scrollWidth,viewportWidth:s.clientWidth,eventLeft:e.left,eventRight:e.right,labelRight:label.right,viewportRight:r.right,overflow:document.documentElement.scrollWidth>innerWidth};});assert(pan.headingLeft>=0&&pan.headingRight<=390);assert(pan.contentWidth<=pan.viewportWidth&&pan.scroll===0&&pan.eventLeft>=pan.labelRight-1&&pan.eventRight<=pan.viewportRight+1&&!pan.overflow);await p.locator('#result-panel').evaluate(e=>e.scrollIntoView({block:'start'}));await p.screenshot({path:out+'/timeline-mobile.png'});
  await p.locator('#save-decision').click();await p.waitForFunction(()=>document.getElementById('decision-status').textContent.includes('您的反馈已保存。'));assert(await p.locator('#feedback-complete').isVisible());assert.strictEqual(x.state().payload.score,3.75);assert.strictEqual(await p.evaluate(()=>Object.keys(localStorage).filter(k=>k.startsWith('eb:decision-draft:')).length),0);reports.push({case:'draft_roundtrip_isolation_submit',passed:true,...x.state(),pan});assert.deepStrictEqual(x.errors,[]);await x.ctx.close();
