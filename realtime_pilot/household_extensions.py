...
HOUSING=[
 ...
 extra('X_CITY','常住城市（选填，无需详细地址）',[],'housing',text=True),
 extra('X_BUILDING','住房的建筑形式更接近哪一种？',[('apartment','公寓 / 多户住宅'),('house','独立 / 联排住宅'),('other','其他住房')],'housing'),
 extra('X_TENURE','目前的居住方式是？',[('owned','自有住房'),('rented','整套租住'),('shared','与他人合租'),('other','其他安排')],'housing'),
 extra('X_AREA','您家这套住房的建筑面积大致为？（合租按整套住房）',[('lt50','不足50㎡'),('50_89','50—不足90㎡'),('90_119','90—不足120㎡'),('120_159','120—不足160㎡'),('ge160','160㎡及以上')],'housing'),
...
CONTEXT=[
 ...
 extra('X_BILL','最近一个夏季月，这些同住家人合计承担的电费大致为？（人民币；其他币种可大致折算）',[('lt100','不足100元'),('100_299','100—不足300元'),('300_599','300—不足600元'),('600_999','600—不足1000元'),('ge1000','1000元及以上')],'context'),
...
for device,label in [('ac','空调'),('washer','洗衣机'),('dishwasher','洗碗机'),('dryer','烘干机'),('home_ev','可在家充电的电动汽车（含插混，不含电动自行车）'),('electric_water_heater','电热水器')]:
    ...
    frequency_prompt=f'夏季通常一周有几天在家为电动汽车充电？（含插混，不含电动自行车）' if device=='home_ev' else f'夏季通常一周有几天使用{label}？'
    INVENTORY.extend([
...
def build_record(profile, questions, *, household_id, raw_answers, questionnaire_version, submitted_at):
    ...
    return {'schema_version':'eb.real_household_record.v1','household_id':household_id,
        ...
        'analysis_features':{'status':'candidate_not_fitted','uses_event_feedback':False,
            'research_answers':{k:deepcopy(v) for k,v in profile.items() if k in research or k=='M_MEMBERS'}},
        'eb_projection':{'version':'eb.real_household_projection.v1',
            'included_question_ids':[q['id'] for q in questions if not q.get('research_only')],
            'research_only_question_ids':list(research),
            'simulation_binding':'shared_research_building_and_tianjin_weather',
            'notes':'Research-only answers do not change model, weather, tariff, device counts or planning input. Member descriptions are visible evidence; no synthetic member weights or votes.'}}
